...
/*******************************************************************************
 LCD_WR_REG: 设置LCD模块上指定寄存器的值。 输入: 寄存器地址、寄存器值。
 			Set the register on lcd module. Input: Register address  ,value
*******************************************************************************/
...
/*******************************************************************************
 delayms: 毫秒（mS）延时程序。 输入: 延时等待的毫秒数值（在72MHz主频情况下）
 		delayms.Input: ms to delay(72M main frequency)
*******************************************************************************/
void Delayms(u16 mS) 
{ 
  Delay_Cnt = mS;
...
/*******************************************************************************
 Point_SCR : 指定显示位置 for R61509V LCD   输入: X、Y坐标 
 			Set the position to show  Input :X.Y position
*******************************************************************************/
...
/*******************************************************************************
 Display_Str: 在指定位置显示字符串   输入: X、Y坐标，颜色值，显示模式，字符串 
 			  Show a char in a specified positon Input:X,Y ,show color, show mode , string to show
*******************************************************************************/
...
{ 
  ...
  x0++;                            // 每一个字符串前增加一空白列 //Add a blank in font of each string
  while (*s!=0) {
    for(i=0;i<8;++i){
      Point_SCR(x0+i, y0);
    ...
    if(*s==0x21) x0 +=4;           //显示位置水平方向+4//+4 in horizontal 
    else  x0 += 8;                 //显示位置水平方向+8//+8 in horizontal
    ++s;                           //字符串指针+1//Pointer to string incease by 1
  }
}
/*******************************************************************************
 Display_Logo: 在指定位置显示商标图案   输入: X、Y坐标 
 				Show logo in a specified position    Input: X,Y potition 
*******************************************************************************/
...
//  u32 b;
  ...
  x = x0;
  y = y0;
  for(i=0; i<16; i++){        // 0x36~0x75: 调色板(每色4B,顺序为B,G,R,Alpha)//Sample color
    Color[i]=((Ptr[0x36 + i*4]>>3)<<11)+
      ((Ptr[0x37 + i*4]>>2)<<5)+(Ptr[0x38 + i*4]>>3);
  }      
  for(i=0; i<256*64/2; ++i){
    Point_SCR(x, y);
    LCD_PORT = Color[(Ptr[0x76 + i]&0xF0)>>4]; // 高半字节   //Upper part 
    x++;
    if(x > x0+255) { x = x0; y++;}
    Point_SCR(x, y);
    LCD_PORT = Color[Ptr[0x76 + i]&0x0F];      // 低半字节// Lower part
    x++;
    if(x > x0+255) { x = x0; y++;}
  }
}
